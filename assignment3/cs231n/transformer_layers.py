...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        #     shape (N, H, T, T). For more examples, see    维度>2 ，只对最后两个维度进行矩阵乘法                       #
        ...
        K = torch.reshape(Key,(N,T,self.n_head,-1))  # (N, T, H, E/H)
        Q = torch.reshape(Query,(N,S,self.n_head,-1)) # (N, S, H, E/H)
        V = torch.reshape(Value,(N,T,self.n_head,-1)) # (N, T, H, E/H)

        K = torch.permute(K,(0,2,1,3)) #(N,H,T,E/H)
        Q = torch.permute(Q,(0,2,1,3)) #(N,H,S,E/H)
        V = torch.permute(V,(0,2,1,3)) #(N,H,T,E/H)

        scores = torch.matmul(Q,K.transpose(-2,-1)) / (self.head_dim**0.5) #（N,H,S,T)
        if attn_mask is not None:
            scores = scores.masked_fill(attn_mask==0,-1e9) #True的地方替换为一个很小的值,masked的位置意味着softmax占比很低
